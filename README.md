...
**ClockBeat**는 심장 박동(ECG) 패턴과 세련된 유체 애니메이션, 그리고 현대적인 **클레이모피즘(Claymorphism)** UI를 특징으로 하는 실시간 시계 웹 애플리케이션입니다.

- **URL**: [https://clock.qus0in.dev/](https://clock.qus0in.dev/)

## 🌟 주요 기능 (Key Features)

### 1. 클레이모피즘 & 모션 (Claymorphism & Motion)
- 애플리케이션 전반에 걸쳐 소프트한 배경, 둥근 곡선(`rounded-[3.5rem]`), 외곽 그림자와 내부 형태 그림자(`inset box-shadow`)를 조합해 밝고 부드러운 점토/플라스틱 같은 입체적 질감을 부여했습니다.
- 테마 토글 버튼, 타임존 전환 버튼 등 사용자가 인터랙션하는 요소 모두에서 클레이모피즘 톤앤매너와 함께 스무스한 트랜지션 애니메이션을 느낄 수 있습니다.
